...
- [Multi-AZ Database Failover (High Availability)](p3-DB-failover.md) – I enabled Multi-AZ deployment for the RDS instance and observed how AWS performs automatic failover to a standby database in another availability zone to maintain availability.
- [Failure testing: Reboot DB & test connectivity](p3-DB-failure-testing.md) – I simulated a real operational event by rebooting the RDS database instance and testing connectivity from EC2 before and after the reboot to observe system recovery behavior.
- [Secure Access & Private Routing](p3-BastionHost&NATgetway.md) – Deployed a bastion host for SSH access to private instances and configured a NAT Gateway to allow outbound internet traffic without exposing private resources.
